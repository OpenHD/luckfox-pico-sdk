 ...
 */
static const struct regval mis5001_linear_10_720p_regs[] = {
...
static const struct mis5001_mode supported_modes[] = {
	{
		...
		.reg_list = mis5001_linear_10_720p_regs,
		.hdr_mode = NO_HDR,
...
{
	return abs((int)mode->width  - (int)framefmt->width) +
	       abs((int)mode->height - (int)framefmt->height);
}

static const struct mis5001_mode *
mis5001_find_best_fit(struct v4l2_subdev_format *fmt)
{
	const struct v4l2_mbus_framefmt *req = &fmt->format;
	int best_ge_idx = -1;
	int best_ge_dist = INT_MAX;
unsigned int i = 0;
	int best_any_idx = -1;   /* fallback: original logic */
	int best_any_dist = INT_MAX;

	for (i = 0; i < ARRAY_SIZE(supported_modes); i++) {
		const struct mis5001_mode *m = &supported_modes[i];
		int dist = mis5001_get_reso_dist(m, req);
		...
		/* Enforce strictly same-or-larger in BOTH dimensions */
		if (m->width >= req->width && m->height >= req->height) {
			if (dist < best_ge_dist) {
				best_ge_dist = dist;
				best_ge_idx = (int)i;
			}
		}
	...
	/* Prefer same-or-larger; otherwise fallback to overall closest */
	return (best_ge_idx >= 0) ? &supported_modes[best_ge_idx]
				  : &supported_modes[best_any_idx];
}
			   ...
			   struct v4l2_subdev_format *fmt)
{
	...
	printk("mis 5001: We are setting %dx%d mode\n", mode->width, mode->height);
